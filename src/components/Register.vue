<template>
  <div class="page__signup">
    ...
    >
      ...
      />
      <small class="invalid-user" v-if="$v.name.$dirty && !$v.name.required"
        >Enter name</small
      >
      <i class=""></i>
      <input
      ...
      >
      <button v-if="loading" type="submit" class="sign-btn">LOADING...</button>
      <button v-else type="submit" class="sign-btn">SIGN UP</button>
    </form>
  </div>
</template>
